...
export async function getDocuments(params?: {
  ...
  const cacheKey = `documents:${params?.category || "all"}:${params?.year || "all"}:${params?.search || "none"}`;
...
export async function getDocumentCategories(): Promise<string[]> {
  return withCache("documents:categories", 60, async () => {
    try {
      const docs = await prisma.document.findMany({
...
export async function getDocumentYears(): Promise<number[]> {
  return withCache("documents:years", 60, async () => {
    try {
      const docs = await prisma.document.findMany({
        select: { year: true },
        distinct: ["year"],
